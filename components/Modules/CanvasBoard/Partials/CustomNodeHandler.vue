...
const getHandles = computed(() => {
  ...
  return getEdges.value
    ...
    .map((edge, _, arr) => {
      ...
      const IsTheSameNode = EdgeId.includes(canvasStore.currentActiveNode.id);
      ...
      const CurrentSpace = MiddlePlace - CoordsSpaceStep * CurrentPosLength;
      ...
      return {
        ...
        cardinalityLabel: IsSource ? Cardinality.source : Cardinality.target,
      };
    });
});
</script>
...
<template>
  <Handle
    ...
    :id="handle.id"
    :key="`${handle}-${ind}`"
    class="h-[10px] w-[10px] rounded-full border-2"
    ...
    :class="{
      ...
      'dark:border-dark-200 dark:bg-dark-600 border-slate-200 bg-slate-500':
        !handle.isHandleActive && !props.isFaded,
      'border-blue-50 bg-blue-600': handle.isHandleActive && !props.isFaded,
      'dark:border-dark-500 dark:bg-dark-800 border-slate-50 bg-slate-300':
        !handle.isHandleActive && props.isFaded,
    }"
  >
    <span
      v-if="handle.isHandleActive && !props.isFaded"
      class="text-md dark:text-dark-50 block font-bold text-slate-950"
      :class="{
        '-translate-y-[20px] translate-x-[10px]':
          handle.position === 'top' || handle.position === 'right',
